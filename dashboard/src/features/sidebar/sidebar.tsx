import { Sidebar, type SidebarItem } from "@marzneshin/common/components";
import { useIsCurrentRoute } from "@marzneshin/common/hooks";
import { useEffect, type FC } from "react";
import {
  sidebarItems as sidebarItemsSudoAdmin,
...
export const DashboardSidebar: FC<DashboardSidebarProps> = ({
  ...
  const sidebarItems = isSudo()
    ? sidebarItemsSudoAdmin
    : sidebarItemsNonSudoAdmin;
              ...
              {Object.keys(sidebarItems).map((key) => (
                <div className="w-full" key={key}>
                  ...
                  {sidebarItems[key].map((item: SidebarItem) => (
                    <Sidebar.Item
                      variant={
                        isCurrentRouteActive(item.to) ? "active" : "default"
                      }
            ...
            <Sidebar.Footer>
              {collapsed ? (
                ...
                <>
                  ...
                  <Card>
                    <CardContent
                      className={cn("p-4 flex flex-col w-fit gap-2")}
                    >
                      <span className="text-xs">Your Balance is</span>
                      <strong className="font-bold">
                        {isSudo() ? "Unlimited" : `${balance.toString()} GB`}
                      </strong>
                    </CardContent>
                  </Card>
                </>
