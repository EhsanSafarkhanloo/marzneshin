...
import { AlertDialog, AlertDialogContent } from '@marzneshin/common/components'

const AdminProvider = () => {
  const { username } = Route.useLoaderData()
  const { data: admin, isPending } = useSuspenseQuery(
    adminQueryOptions({ username }),
  )
...
export const Route = createFileRoute('/_dashboard/admins copy/$adminId')({
  loader: async ({ params }) => {
